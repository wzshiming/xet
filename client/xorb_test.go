...
func TestHasXorbStalledHeadersFailAfterRetries(t *testing.T) {
	...
	c, err := NewClient(WithHTTPClient(countingClient(&attempts)), WithIdleTimeout(50*time.Millisecond), WithRetries(1), WithUpstreamProvider(StaticUpstreamProvider(srv.URL, "")))
	if err != nil {
		t.Fatal(err)
	}
	ctx, cancel := context.WithTimeout(t.Context(), 5*time.Second)
...
func TestRawGETIdleTimeout(t *testing.T) {
	...
	c, err := NewClient(WithIdleTimeout(50*time.Millisecond), WithRetries(3), WithUpstreamProvider(StaticUpstreamProvider(srv.URL, "")))
	if err != nil {
		t.Fatal(err)
	}
	ctx, cancel := context.WithTimeout(t.Context(), 5*time.Second)
