...
func TestQueryChunkDeduplicationFound(t *testing.T) {
	...
	if result.IsNew {
		t.Error("Expected IsNew=false for found chunk")
	}
	if result.XorbHash != xorbHash {
		t.Errorf("unexpected xorb hash: got %s want %s", result.XorbHash, xorbHash)
	}
}
